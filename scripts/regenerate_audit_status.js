...
const FV_AUDIT = {
  ...
  rows: [
    ...
    { catalog: 'NAPRA_ODB_DATA (drug scheduling / ODB coverage)', entries: 1547, coverage: '≈ 1,295 / 1,547 (~84%)', status: 'IN PROGRESS — first pass',
      evidence: 'Batches 1-15 (2026-05-17): first ~1,295 entries reviewed (antimicrobials, cardiovascular, antithrombotic, endocrine incl. bone agents, psychiatric, GI, respiratory, analgesic, dermatology, ophthalmic/otic/nasal, immune globulin, addiction medicine, NRT, neurology, HIV/HCV/HBV antivirals, antifungals, TB drugs, antiparasitics/antimalarials, hospital antibiotics, antihistamines/decongestants, immunology biologics, oncology incl. ICIs/targeted agents, OAB/alpha-blockers, obstetric agents, iron salts, OTC topicals, natural health products, insulin, antiseizure, statins/fibrates, ESAs/G-CSF, antiarrhythmics). 10 fixes — ceftriaxone gonorrhea dose; naproxen + pyrantel entries reconstructed; "triplicate prescription required in Ontario" removed from 3 stimulant entries + sodium_oxybate (Ontario uses the Narcotics Monitoring System, not triplicate pads); glecaprevir/pibrentasvir Maviret duration; phenobarbital mislabeled "Targeted Substance" → controlled drug; epoetin_alfa typo + luCode contradiction; sodium_oxybate CDSA Schedule III→I correction. 4 flagged for verification (ulipristal_acetate, mometasone_nasal, timolol_ophthalmic duplicate, nabilone CDSA Schedule II-vs-III contradiction). Remaining ~252 entries pending batched review.' },
...
lines.push('');
lines.push('The audit script measures **structural** integrity (schema, depth thresholds, cross-references, taxonomy, dates). It cannot measure:');
lines.push('');
lines.push('- **Clinical accuracy** of pearls, interactions, monitoring, contraindications, indications.');
lines.push('- **Currency** of Health Canada approval status — newly approved drugs, withdrawn drugs, monograph revisions, label changes.');
lines.push('- **Severity correctness** on drug interactions (the script checks the severity is one of 6 canonical values, but cannot verify a specific interaction was correctly classified as Major vs Moderate).');
lines.push('- **Canadian-context correctness** — the script accepts any token from a Canadian-keyword list, but cannot verify the citation actually supports the clinical content.');
lines.push('- **Dose accuracy** for renal/hepatic/pediatric/elderly adjustments.');
lines.push('- **Pregnancy-category correctness** beyond presence of the structured field.');
